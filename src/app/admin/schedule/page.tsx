...
export default function AdminSchedulePage() {
  const [classes, setClasses] = useState<ScheduledClass[]>(seedSchedule);
  const [instructorRoster, setInstructorRoster] = useState(defaultInstructors);
    ...
    <main className="flex-1 bg-bg">
      <div className="mx-auto max-w-5xl px-7 py-8">
        ...
        {editingId && form && (
          <div className="mt-5 rounded-2xl border-2 border-accent bg-surface p-5">
            ...
            <div className="mt-4 flex gap-2.5">
              <button
                onClick={saveForm}
                className="rounded-[9px] bg-accent-strong px-4 py-2.5 text-[13px] font-bold text-accent-ink hover:brightness-110"
              >
                Save class
              </button>
              <button
        ...
        <p className="mt-8 text-[11.5px] text-muted">
          These edits live only in this browser tab for now — once the database is
          connected, they&rsquo;ll save for real and show up for clients immediately.
        </p>
      </div>
    </main>
  );
